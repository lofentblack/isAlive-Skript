...
if [ "$(id -u)" != "0" ]; then
  echo $(tput setaf 1)Bitte wechsel zu einem Root Benutzer$(tput sgr0)
  exit 1;
fi
...
Port_richtigkeits_Test() {
	...
	echo $rot
	echo "Mit dem download akzeptierst du die Lizenz von lofentblack.de/licence"
	echo "$(tput sgr0)"
...
if [ -f isAlive.config ]; then
	...
	echo -e '#################################################################################
##                                                                             ##
## Du musst unten immer ein Port und ein Port Dienst eingeben.               ##
...
LOGO() {
	...
	echo "5) Beenden"
	read -p "Was willst möchten du machen? " machen
}
  ...
  if [ -s $SCRIPTPATH/$LOCK ]; then
	...
	if [ $machen == "1" ]; then
		...
		read -p "Bist du dir sicher? (Y/N) " sicherheit
